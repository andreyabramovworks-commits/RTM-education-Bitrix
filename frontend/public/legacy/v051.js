...
(function () {
  ...
  function mountTakeCanvas(test) {
    ...
    var meta = normalizeMeta(j(test.PROPERTY_VALUES.meta)), returned = userAttempt(test.ID, ['returned']), existing = returned && returned.PROPERTY_VALUES && returned.PROPERTY_VALUES.answers;
    try { takeAnswers = existing ? JSON.parse(existing) : {}; } catch (_) { takeAnswers = {}; }
  ...
  window.takeTestSubmit = takeTestSubmit = async function (event) {
    ...
    document.getElementById('v51OutcomeNext').onclick = function () { closeModal(); if (autoPassed) adjacentMat(1); else openUserMaterial(test); };
