...
(function () {
  ...
  var workspaceTimer = 0, workspaceScene = null, workspaceRevision = 0, workspaceMounted = false, workspaceRestoring = false, workspaceGeneration = 0, developerPreviewRole = null, testUiChoice = 'modern';
  ...
  async function mountWorkspace() {
    ...
    try { var payload = await window.RTMV47.request('/api/v47/developer-workspace'); if (generation !== workspaceGeneration) return; workspaceScene = payload.scene; workspaceRevision = Number(payload.revision || 0); workspaceMounted = true; workspaceRestoring = false; var status = root.querySelector('.v492-workspace-status'); function save() { if (workspaceRestoring || generation !== workspaceGeneration) return; clearTimeout(workspaceTimer); workspaceTimer = setTimeout(async function () { if (workspaceRestoring || generation !== workspaceGeneration) return; status.textContent = 'Сохраняю…'; try { var saved = await window.RTMV47.request('/api/v47/developer-workspace', {method: 'PUT', body: JSON.stringify({scene: workspaceScene})}); if (generation !== workspaceGeneration) return; workspaceRevision = Number(saved.revision || workspaceRevision + 1); status.textContent = 'Сохранено на сервере · ревизия ' + workspaceRevision + ' · резервная копия выгружается в Google Drive автоматически'; } catch (error) { status.textContent = 'Не удалось отправить на сервер. Лист открыт, повторю при следующем изменении.'; } }, 1400); } window.RTMCanvas.mount(host, {pageKey: 'developer-workspace:'+workspaceRevision, scene: workspaceScene, readOnly: false, completionRequired: false, title: '', brandColor: '#ef174c', onChange: function (scene) { if (workspaceRestoring || generation !== workspaceGeneration) return; workspaceScene = scene; save(); }, onManualSave: async function () { if (workspaceRestoring || generation !== workspaceGeneration) return; clearTimeout(workspaceTimer); var saved = await window.RTMV47.request('/api/v47/developer-workspace', {method: 'PUT', body: JSON.stringify({scene: workspaceScene})}); if (generation !== workspaceGeneration) return; workspaceRevision = Number(saved.revision || workspaceRevision + 1); status.textContent = 'Сохранено на сервере · ревизия ' + workspaceRevision; }, onRequestDisk: window.RTMV46.pickDiskMedia}); status.textContent = 'Лист загружен · ревизия ' + workspaceRevision + ' · автосохранение включено'; } catch (error) { workspaceRestoring = false; root.innerHTML = '<div class="v43-canvas-error"><b>Лист пока недоступен</b><span>' + esc(error.message || String(error)) + '</span><button type="button">Повторить</button></div>'; root.querySelector('button').onclick = mountWorkspace; }
  }
  ...
  window.RTMV492 = {mountWorkspace: mountWorkspace, restoreWorkspace: restoreWorkspace, bindTestSwitch: bindTestSwitch, previewRole: function (role) { if (!isActualDeveloper()) return; applyDeveloperPreview(role); }};
})();
