...
function DocumentComposer({ bridge, document, onBack }) {
  ...
  const spanStyle = (style = {}) => ({ fontWeight: style.bold ? 700 : undefined, fontStyle: style.italic ? "italic" : undefined, textDecoration: [style.underline && "underline", style.strikethrough && "line-through"].filter(Boolean).join(" ") || undefined, color: style.color || undefined, fontFamily: style.fontFamily || undefined, fontSize: style.fontSize ? `${style.fontSize}pt` : undefined });
  ...
  const image = (item, key, inline = false) => <figure className={inline ? "dc-inline-island" : "dc-visual-island"} key={key} style={{ maxWidth: item.width ? `${item.width}pt` : undefined }}>{item.assetUrl ? <img src={item.assetUrl} alt="" /> : <span>Изображение недоступно</span>}</figure>;
  const body = (item) => <>{(item.spans || []).map((span, index) => { const text = <span key={index} style={spanStyle(span.style)}>{span.text}</span>; return span.style?.link ? <a key={index} href={span.style.link} target="_blank" rel="noreferrer">{text}</a> : text; })}{(item.images || []).map((item, index) => image(item, `image-${index}`, true))}</>;
  const single = (item, key) => { if (item.kind === "image") return image(item, key); if (item.kind === "table") return <div className="dc-table-wrap" key={key}><table><tbody>{(item.rows || []).map((row, rowIndex) => <tr key={rowIndex}>{row.map((cell, cellIndex) => <td key={cellIndex}>{cell.map((part, partIndex) => <React.Fragment key={partIndex}>{body(part)}</React.Fragment>)}</td>)}</tr>)}</tbody></table></div>; const Tag = item.kind === "heading" ? `h${Math.min(4, Math.max(1, item.level || 2))}` : "p"; return <Tag key={key} style={blockStyle(item.style)}>{body(item)}</Tag>; };
  const blocks = (items) => { const result = [], list = []; const flush = () => { if (!list.length) return; const first = list[0].list, Tag = first.type === "ordered" ? "ol" : "ul"; result.push(<Tag className={`dc-list dc-list-${first.type} dc-list-level-${first.level || 0}`} start={first.type === "ordered" ? first.start : undefined} key={`list-${result.length}`}>{list.map((item, index) => <li key={index}>{single({ ...item, list: undefined }, `list-item-${index}`)}</li>)}</Tag>); list.length = 0; }; for (let index = 0; index < items.length; index += 1) { const item = items[index]; if (item.region === "header" || item.region === "closing") { flush(); const region = item.region, grouped = [item]; while (items[index + 1]?.region === region) grouped.push(items[++index]); result.push(<section className={`dc-document-${region}`} key={`${region}-${index}`}>{grouped.map((block, blockIndex) => single(block, blockIndex))}</section>); continue; } if (item.kind === "image") { flush(); const gallery = [item]; while (items[index + 1]?.kind === "image") gallery.push(items[++index]); const caption = items[index + 1]?.kind === "paragraph" || items[index + 1]?.kind === "heading" ? items[++index] : null; result.push(<figure className={`dc-gallery dc-gallery-${Math.min(gallery.length, 3)}`} key={`gallery-${index}`}>{gallery.map((picture, pictureIndex) => image(picture, pictureIndex))}</figure>); if (caption) result.push(<div className="dc-gallery-caption" key={`caption-${index}`}>{body(caption)}</div>); continue; } if (item.list) { const previous = list[list.length - 1]; if (previous && (previous.list.id !== item.list.id || previous.list.level !== item.list.level)) flush(); list.push(item); } else { flush(); result.push(single(item, index)); } } flush(); return result; };
  ...
  return <main className={`dc-shell ${pageMode ? "is-page-mode" : ""}`}><header className="dc-topbar"><div><small>База знаний</small><strong>{state.data.title}</strong></div><button className="dc-mode-toggle" aria-label={pageMode ? "Перейти к непрерывному чтению" : "Показать страницы"} title={pageMode ? "Непрерывное чтение" : "Показать страницы"} aria-pressed={pageMode} onClick={() => setPageMode((value) => !value)}><Icon name="pages" /></button></header><div className="dc-page-progress" aria-live="polite">{currentPage} / {pages.length}</div><div className="dc-reader" ref={readerRef}>{pages.map((page) => <article className="dc-page" data-page={page.number} key={page.number}>{blocks(page.blocks || [])}<div className="dc-page-corner">{page.number}</div></article>)}</div><button className="dc-mobile-back" onClick={onBack}><Icon name="back" />Назад</button></main>;
}
