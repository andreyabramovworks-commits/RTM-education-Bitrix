...
function DocumentComposer({ bridge, document, onBack }) {
  ...
  useEffect(() => {
    ...
    const start = (event) => { if (event.touches.length === 2) pinchRef.current = { distance: distance(event.touches), zoom: zoomRef.current }; };
    const move = (event) => { const pinch = pinchRef.current; if (!pinch || event.touches.length !== 2) return; event.preventDefault(); setZoom(Math.min(2.25, Math.max(1, pinch.zoom * distance(event.touches) / pinch.distance))); };
