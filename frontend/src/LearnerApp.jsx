...
function DocumentComposer({ bridge, document, onBack }) {
  ...
  const body = (item) => <>{(item.spans || []).map((span, index) => { const text = <span key={index} style={spanStyle(span.style, item.kind === "heading")}>{span.text}</span>; return span.style?.link ? <a key={index} href={span.style.link} target="_blank" rel="noreferrer">{text}</a> : text; })}{(item.images || []).map((item, index) => image(item, `image-${index}`, true))}</>;
  const single = (item, key, fitCell = false) => { if (item.kind === "image") return image(item, key, false, fitCell); if (item.kind === "image-group") return <figure className={`dc-gallery dc-gallery-${Math.min(item.images.length, 4)}`} key={key}>{item.images.map((picture, index) => image(picture, index, false, fitCell))}</figure>; if (item.kind === "table") { const cells = (item.rows || []).flat(); const partsOf = (cell) => cell.items || cell; const visual = cells.length > 0 && cells.every((cell) => partsOf(cell).every((part) => part.kind === "image" || part.kind === "image-group" || (part.kind === "paragraph" && !part.list))); return <div className={`dc-table-wrap ${visual ? "is-visual-table" : ""} ${item.hasBorders ? "has-borders" : ""}`} key={key}><table><tbody>{(item.rows || []).map((row, rowIndex) => <tr key={rowIndex}>{row.map((cell, cellIndex) => <td key={cellIndex} colSpan={cell.colSpan || undefined} rowSpan={cell.rowSpan || undefined}>{partsOf(cell).map((part, partIndex) => <React.Fragment key={partIndex}>{single(part, `cell-${rowIndex}-${cellIndex}-${partIndex}`, visual)}</React.Fragment>)}</td>)}</tr>)}</tbody></table></div>; } const Tag = item.kind === "heading" ? `h${Math.min(4, Math.max(1, item.level || 2))}` : "p"; return <Tag key={key} style={blockStyle(item.style)}>{body(item)}</Tag>; };
