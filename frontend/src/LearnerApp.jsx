...
function Knowledge({ bridge, openKnowledgeMaterial, hintsEnabled }) {
  ...
  return <main className="lr-page"><header className="lr-page-head lr-kb-head"><div><span className="lr-eyebrow">Справочник</span><h1>База знаний {hintsEnabled && <Hint bridge={bridge} hintKey="knowledge-base" label="База знаний" />}</h1><p>Структура документов компании, статьи, тесты и история редакций.</p></div><label className="lr-search"><span>Поиск по базе знаний</span><input value={query} onChange={(event) => { setQuery(event.target.value); setSelected(null); }} placeholder="Название или ключевое слово" /></label></header><ResourceState state={state} retry={() => load(true)}>{({ tree, documents }) => {
    ...
    if (selected) return <section className="lr-kb-detail">
      <button className="lr-back" onClick={() => setSelected(null)}>← Назад к структуре</button>
      <h2>{selected.title}</h2>
      {selected.description && <p>{selected.description}</p>}
      <div className="lr-kb-actions">{selected.documentUrl && <a className="lr-secondary" href={selected.documentUrl} target="_blank" rel="noreferrer">Открыть оригинал</a>}<button className="lr-primary" onClick={() => openKnowledgeMaterial(selected, "article")}>Открыть статью</button>{selected.lightTest?.created && <button className="lr-secondary" onClick={() => openKnowledgeMaterial(selected, "light")}>Тест лайт</button>}{selected.fullTest?.created && <button className="lr-secondary" onClick={() => openKnowledgeMaterial(selected, "full")}>Полный тест</button>}</div>
      <section className="lr-edition-history"><h3>История редакций</h3>{editionsState.loading ? <div className="lr-resource-state">Загружаем историю…</div> : editionsState.error ? <p className="lr-inline-error">{editionsState.error}</p> : editionsState.data.length ? editionsState.data.map((edition) => <article key={edition.id}><b>Редакция от {formatDate(edition.editionDate)}</b>{edition.googleVersionName && <small>{edition.googleVersionName}</small>}<p>{edition.changeLog || "Изменения не описаны"}</p></article>) : <p className="lr-muted">История редакций пока пуста.</p>}</section>
    </section>;
    return <><div className="lr-kb-crumbs"><button onClick={() => { setPath([]); setQuery(""); }}>База знаний</button>{path.map((id, index) => { const part = findTreeNode(tree, path.slice(0, index + 1)); return <React.Fragment key={id}><span>›</span><button onClick={() => setPath(path.slice(0, index + 1))}>{part?.title}</button></React.Fragment>; })}</div><div className="lr-section-heading"><div><span className="lr-eyebrow">{q ? "Результаты поиска" : "Раздел"}</span><h2>{q ? `По запросу «${query.trim()}»` : node?.title || "База знаний"}</h2></div></div>{rows.length ? <div className="lr-kb-grid">{rows.map((item) => { const doc = item.type === "material" ? byRow.get(Number(item.row)) : null; const count = item.type === "folder" ? flattenMaterials(item).length : 0; return <button className="lr-kb-card" key={item.id} onClick={() => item.type === "folder" ? setPath([...path, item.id]) : doc && setSelected({ ...doc, title: item.title || doc.title })}><span className="lr-kb-icon"><Icon name={item.type === "folder" ? "folder" : "article"} /></span><span className="lr-kb-copy"><strong>{item.title}</strong><small>{item.type === "folder" ? `${count} ${count === 1 ? "материал" : count < 5 ? "материала" : "материалов"}` : "Статья, тесты и история редакций"}</small></span><span className="lr-kb-arrow"><Icon name="arrow" /></span></button>; })}</div> : <EmptyState title="Ничего не найдено">Измените запрос или вернитесь на уровень выше.</EmptyState>}</>;
  }}</ResourceState></main>;
}
