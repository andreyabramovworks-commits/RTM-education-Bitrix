...
function DocumentComposer({ bridge, document, onBack }) {
  ...
  const image = (item, key, inline = false) => <figure className={inline ? "dc-inline-island" : "dc-visual-island"} key={key} style={{ maxWidth: item.width ? `${item.width}pt` : undefined }}>{item.assetUrl ? <img src={item.assetUrl} alt={item.alt || ""} /> : <span>Иллюстрация недоступна</span>}<figcaption>{item.alt}</figcaption></figure>;
