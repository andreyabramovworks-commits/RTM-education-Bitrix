...
function DocumentComposer({ bridge, document, onBack }) {
  ...
  const single = (item, key) => { if (item.kind === "image") return image(item, key); if (item.kind === "image-group") return <figure className={`dc-gallery dc-gallery-${Math.min(item.images.length, 4)}`} key={key}>{item.images.map((picture, index) => image(picture, index))}</figure>; if (item.kind === "table") { const cells = (item.rows || []).flat(); const visual = cells.length > 0 && cells.every((cell) => cell.every((part) => part.kind === "image" || part.kind === "image-group" || (part.kind === "paragraph" && !part.list))); return <div className={`dc-table-wrap ${visual ? "is-visual-table" : ""}`} key={key}><table><tbody>{(item.rows || []).map((row, rowIndex) => <tr key={rowIndex}>{row.map((cell, cellIndex) => <td key={cellIndex}>{cell.map((part, partIndex) => <React.Fragment key={partIndex}>{single(part, `cell-${rowIndex}-${cellIndex}-${partIndex}`)}</React.Fragment>)}</td>)}</tr>)}</tbody></table></div>; } const Tag = item.kind === "heading" ? `h${Math.min(4, Math.max(1, item.level || 2))}` : "p"; return <Tag key={key} style={blockStyle(item.style)}>{body(item)}</Tag>; };
