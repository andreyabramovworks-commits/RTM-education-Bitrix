...
export function LearnerApp({ active, bridge, onSetMode }) {
  ...
  if (view === "videos" && !materialContext) return <div className="learner-app" style={style}><header className="lr-header"><button className="lr-brand" onClick={() => go("learn")} aria-label="На главную">{appearance.logo && <img src={appearance.logo} alt="" />}<span className="lr-brand-name">{appearance.brandName || "RTM обучение"}</span></button><nav className="lr-nav" aria-label="Основная навигация">{NAV.map(([id,label]) => <button key={id} className={view === id ? "active" : ""} aria-current={view === id ? "page" : undefined} onClick={() => go(id)}><Icon name={id}/>{label}</button>)}</nav><div className="lr-header-actions"><button className="lr-hints" onClick={setHints} aria-label="Подсказки"><Icon name="help"/><span>Подсказки</span></button><button className="lr-menu-button" aria-label="Открыть меню" onClick={() => setMenu(!menu)}><Icon name={menu ? "close" : "menu"}/></button></div></header>{menu && <nav className="lr-mobile-nav" aria-label="Мобильная навигация">{NAV.map(([id,label]) => <button key={id} className={view === id ? "active" : ""} onClick={() => go(id)}><Icon name={id}/>{label}</button>)}</nav>}<VideoLibrary /></div>;
