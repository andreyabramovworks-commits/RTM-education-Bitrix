...
function DocumentComposer({ bridge, document, onBack }) {
  ...
  const single = (item, key, fitCell = false) => { if (item.kind === "image") return image(item, key, false, fitCell); if (item.kind === "image-group") return <figure className={`dc-gallery dc-gallery-${Math.min(item.images.length, 4)}`} key={key}>{item.images.map((picture, index) => image(picture, index, false, fitCell))}</figure>; if (item.kind === "table") { const cells = (item.rows || []).flat(); const partsOf = (cell) => cell.items || cell; const tableParts = cells.flatMap(partsOf); const pictureOf = (part) => part.kind === "image" ? part : part.kind === "image-group" ? part.images?.[0] : part.images?.[0]; const aspectOf = (cell) => { const picture = partsOf(cell).map(pictureOf).find(Boolean); return picture?.width && picture?.height ? picture.width / picture.height : 0; }; const hasImage = tableParts.some((part) => part.kind === "image" || part.kind === "image-group" || part.images?.length); const visual = hasImage && tableParts.every((part) => part.kind === "image" || part.kind === "image-group" || (part.kind === "paragraph" && !part.list)); return <div className={`dc-table-wrap ${visual ? "is-visual-table" : ""} ${item.hasBorders ? "has-borders" : ""}`} key={key}><table><tbody>{(item.rows || []).map((row, rowIndex) => { const totalAspect = row.reduce((total, cell) => total + aspectOf(cell), 0); return <tr key={rowIndex}>{row.map((cell, cellIndex) => { const aspect = aspectOf(cell); return <td key={cellIndex} colSpan={cell.colSpan || undefined} rowSpan={cell.rowSpan || undefined} style={visual && aspect && totalAspect ? { width: `${(aspect / totalAspect) * 100}%` } : undefined}>{partsOf(cell).map((part, partIndex) => <React.Fragment key={partIndex}>{single(part, `cell-${rowIndex}-${cellIndex}-${partIndex}`, visual)}</React.Fragment>)}</td>; })}</tr>; })}</tbody></table></div>; } const Tag = item.kind === "heading" ? `h${Math.min(4, Math.max(1, item.level || 2))}` : "p"; return <Tag key={key} style={blockStyle(item.style)}>{body(item, fitCell)}</Tag>; };
