...
function createLearnerPreviewBridge() {
  ...
  learner.admin = { getSnapshot: () => ({ ...snapshot, route, roleLabel: "Разработчик" }), subscribe: learner.subscribe, refresh: async () => {}, setMode: learner.setMode, openClassic: () => {}, mount: (host) => { const root = document.getElementById("adminApp"), main = root?.querySelector(".admin-main"); if (!main || !host) return; mainHome ||= { parent: main.parentNode, next: main.nextSibling }; adminRootState ||= { hidden: root.hidden, inert: root.inert, ariaHidden: root.getAttribute("aria-hidden") }; host.appendChild(main); root.hidden = true; root.inert = true; root.setAttribute("aria-hidden", "true"); adminRoot = root; mount = host; }, unmount: () => { const main = mount?.querySelector(":scope > .admin-main"); if (main && mainHome) mainHome.parent.insertBefore(main, mainHome.next); if (adminRoot && adminRootState) { adminRoot.hidden = adminRootState.hidden; adminRoot.inert = adminRootState.inert; if (adminRootState.ariaHidden === null) adminRoot.removeAttribute("aria-hidden"); else adminRoot.setAttribute("aria-hidden", adminRootState.ariaHidden); } adminRoot = null; adminRootState = null; mount = null; }, openRoute: (next) => { route = next; mount?.querySelectorAll(".admin-view").forEach((node) => node.classList.toggle("active", node.id === `admin${next[0].toUpperCase()}${next.slice(1)}`)); emit({}); } };
  return learner;
}
