...
test("Document Composer keeps one mobile return action and uses semantic page controls", () => {
  ...
  assert.match(learnerApp, /addEventListener\("pointermove", move, \{ passive: false \}\)/);
  assert.match(learnerApp, /clampReaderZoom/);
  ...
  assert.match(learnerApp, /setReaderZoom\(pinch\.zoom/);
  assert.match(learnerApp, /stage\.dataset\.zoomed = "true"/);
  assert.match(learnerApp, /stage\.dataset\.zoomed = "false"/);
  assert.match(learnerApp, /setZoom\(nextZoom\)/);
  ...
  assert.match(learnerApp, /dc-figure-set/);
  assert.match(learnerApp, /image-group/);
  assert.match(learnerApp, /is-visual-table/);
  assert.match(learnerApp, /const visualFigure/);
  assert.match(learnerApp, /isCaptionBlock/);
  ...
  assert.match(learnerCss, /dc-reader-viewport \{ position:relative; flex:1 1 auto; min-height:0; overflow:auto; overscroll-behavior:contain; touch-action:none/);
  assert.match(learnerCss, /transform:scale\(var\(--dc-reader-zoom\)\)/);
  assert.match(learnerCss, /dc-reader-stage\[data-zoomed="true"\]/);
  assert.doesNotMatch(learnerCss, /zoom:var\(--dc-reader-zoom/);
  assert.match(learnerCss, /dc-table-wrap table \{ width:100%; min-width:600px/);
