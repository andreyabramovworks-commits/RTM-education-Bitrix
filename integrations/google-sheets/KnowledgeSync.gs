/** RTM v50.3.6. Bind syncKnowledgeBase to a developer-only sheet button. */
...
function syncKnowledgeBase(){
  ...
  sheet.getRange(1,12,1,7).setValues([['Статья документа','Тесты документа','Кому назначена статья','Кому назначен тест лайт','Кому назначен тест полный','Проверяющие','Редакторы']]);
  const values=sheet.getRange(2,1,last-1,18).getDisplayValues(),rows=[];
  values.forEach(function(r,i){var link=(r[6].match(/https:\/\/docs\.google\.com\/document\/d\/[^\s,;]+/)||[])[0];if(!link)return;rows.push({row:i+2,title:r[1],description:r[2],documentUrl:link,articleAssignments:parseList_(r[13]),lightTestAssignments:parseList_(r[14]),fullTestAssignments:parseList_(r[15]),reviewers:parseList_(r[16]),editors:parseList_(r[17])});});
  ...
  docs.forEach(function(d){sheet.getRange(d.sourceRow,12,1,7).setValues([[RTM_API+'/documents/'+d.id,(d.lightTest.title||'')+'; '+(d.fullTest.title||''),formatList_(d.articleAssignments),formatList_(d.lightTestAssignments),formatList_(d.fullTestAssignments),formatList_(d.reviewers),formatList_(d.editors)]]);});
