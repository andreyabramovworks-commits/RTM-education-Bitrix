...
def test_bitrix_shell_is_never_cached_and_pins_current_release() -> None:
    ...
    assert response.headers["location"] == "/?bitrix_frame=1&rtm_release=53.1.1"
...
def test_bitrix_shell_preserves_only_safe_application_routes() -> None:
    ...
    assert response.status_code == 303
    assert response.headers["location"] == "/?bitrix_frame=1&rtm_release=53.1.1&rtm_assignment=17&rtm_view=acknowledgements"
