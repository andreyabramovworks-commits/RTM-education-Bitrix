...
<body data-mode="{mode}">
  {('<section class="install"><h1>Подключаем RTM Education</h1><p>Проверяем связь с порталом и сервером. На этом этапе приложение только читает данные текущего пользователя и ничего не записывает в Битрикс24.</p><div class="checks" id="checks"></div><p id="details"></p></section>' if install else '<div hidden><div id="checks"></div><div id="details"></div></div><main class="workspace"><iframe src="/?bitrix_frame=1&amp;rtm_release=49.2.12" title="RTM Education v49.2"></iframe></main>')}
  <script>
    (() => {{
